		...
		if(state == STATE_TYPE) {
			...
			if(input->hasBytes(currentLength)) {
				switch(currentLength) {
					...
					case 2:
						listener->OnInteger(-(int)input->getShort());
						Cborpackage +=  (int) input->getShort();
                        Cborpackage += commaChar;
						Serial.print("h");
